...
This checklist records the local-dev milestone verified on 2026-06-07 with LAN host `192.168.0.160`.
...
- The laptop browser and phone can reach the same LAN host on the local network.

## Start And Automated Verification

Discover the current LAN host:
...
1. Open `http://$LOCAL_HOST:5555/` on the laptop.
...
1. Open `http://<LAN-IP>:5555/` on the phone, using the same host value assigned to `LOCAL_HOST`.
2. Click **Login with VerusID**.
3. Tap **Open Wallet**.
4. Approve the request in Verus Mobile.
...
- Stack status: run `LOCAL_HOST=$LOCAL_HOST ./scripts/status-stack.sh`.
...
- Clean local Docker state: run `./scripts/reset-stack.sh`, then rerun `LOCAL_HOST=$LOCAL_HOST ./scripts/start-stack.sh`.
